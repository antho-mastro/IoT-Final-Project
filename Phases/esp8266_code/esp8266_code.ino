...
const char* ssid = "Mastro14";
const char* password = "Mastro2019";
...
int lcdColumns = 16;
int lcdRows = 2;
String messageStatic = "Dashboard";



void setup_wifi() {
  delay(10);
  //connecting to a WiFi network (iphone)
  Serial.println();
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("mastroDashboard")) {
...
void loop() {
  ...
  if (!client.loop())
    client.connect("mastroDashboard");
  ...
  if (now - lastMeasure > 5000) {
    ...
    Serial.print(humi);
    Serial.println("");
    String hs = "Humidity: " + String((int)humi) + " % ";
    String ts = "Temp: " + String((int)tem) + " C ";
    lcd.setCursor(2, 0);
    lcd.print(messageStatic);
  
    lcd.setCursor(0, 1);
    lcd.print("T:" + String((int)tem) + "C H:" + String((int)humi) + "% L:" + String((int)t));
